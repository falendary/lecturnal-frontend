"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
Object.defineProperty(exports, "__esModule", { value: true });
var core_1 = require("@angular/core");
var slide_repository_1 = require("../repositories/slide.repository");
var SlideService = (function () {
    function SlideService(slideRepository) {
        this.slideRepository = slideRepository;
    }
    SlideService.prototype.getList = function (parameters, presentationId) {
        return this.slideRepository.getList(parameters, presentationId);
    };
    SlideService.prototype.getByKey = function (presentationId, slideId) {
        return this.slideRepository.getByKey(presentationId, slideId);
    };
    SlideService.prototype.update = function (item, presentationId, slideId) {
        return this.slideRepository.update(item, presentationId, slideId);
    };
    SlideService.prototype.create = function (item, presentationId) {
        return this.slideRepository.create(item, presentationId);
    };
    SlideService.prototype.deleteByKey = function (presentationId, slideId) {
        this.slideRepository.deleteByKey(presentationId, slideId);
    };
    return SlideService;
}());
SlideService = __decorate([
    core_1.Injectable(),
    __metadata("design:paramtypes", [slide_repository_1.SlideRepository])
], SlideService);
exports.SlideService = SlideService;

//# sourceMappingURL=slide.service.js.map
